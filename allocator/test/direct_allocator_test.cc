...
public:
  WrapMemBlock(size_t nbytes, DirectAllocator &allocator)
      : allocator(allocator), nbytes(nbytes) {
    mem_block = allocator.Alloc(nbytes, 512, 0, 0);
    // LOG(INFO) << "Allocated " << nbytes << " bytes at " << mem_block << ".";
    total_nbytes += nbytes;
  }
  ~WrapMemBlock() {
    allocator.Free(mem_block, 0);
...
void run(const PagesPoolConf &config, const std::string &name, int seed) {
  SharableObject<PagesPool> pages_pool{config.shm_name, config.shm_nbytes,
                                       config};
  DirectAllocatorConfig caching_allocator_config{.log_prefix = "CA ",
                                                  .shm_name = "test_ca",
                                                  .shm_nbytes = 1_GB,
                                                  .va_range_scale = 8,
                                                  .belong_name = name,
                                                  ...
                                                  .align_nbytes = 512_B};
  // DirectAllocator::RemoveShm(caching_allocator_config);
  SharableObject<DirectAllocator> caching_allocator{
