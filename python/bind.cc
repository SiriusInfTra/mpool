...
// CachingAllocator *CreateCachingAllocator(PagesPool *page_pool,
// CachingAllocatorConfig conf) {
...
PYBIND11_MODULE(_C, m) {
  ...
  py::class_<CachingAllocator>(m, "C_CachingAllocator")
      ...
      .def("RegisterAsPyTorchAllocator", [](CachingAllocator &allocator) {
        UseTorchAllocator(&allocator);
      });
  // py::class_<BelongImpl, std::unique_ptr<Belong, py::nodelete>>(m,
  // "BelongImpl")
  //             .def(py::init<>());
  ...
  m.def("RegisterTorchAllocator", UseTorchAllocator);
  m.def("UnregisterTorchAllocator", UseDefaultAllocator);
